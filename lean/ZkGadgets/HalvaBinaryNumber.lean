...
def Circuit.isValid (c: Circuit P P_Prime) : Prop :=
  ...
  ∀ advice1 advice2: ℕ → ℕ → ZMod P, ∀ phase: ℕ,
    (∀ row col, (col < 2 ∧ c.AdvicePhase col ≤ phase) → advice1 col row = advice2 col row) →
...
theorem soundness (c: ValidCircuit P P_Prime) (h: meets_constraints c): Spec c := by
  ...
  obtain ⟨_, _, _, _, _, _, hgates, _, _, _, _⟩ := h
  unfold all_gates at hgates
  obtain ⟨hg0, hg1, hg2, hg3⟩ := hgates
  unfold gate_0 at hg0
  unfold gate_1 at hg1
  unfold gate_2 at hg2
  unfold gate_3 at hg3
  intro row hf
  refine ⟨?_, ?_, ?_, ?_⟩
  · -- bit 0 is boolean: the field has no zero divisors, so split the product
    have hr := hg0 row
    rw [hf, one_mul] at hr
  ...
  · -- bit 1 is boolean
    ...
    · left; linear_combination h
    · right; linear_combination -h
  · -- value cell is the little-endian recomposition 2·b₀ + b₁
    have hr := hg2 row
    rw [hf] at hr
    linear_combination -hr
  · -- the two bits are never both set
    have hr := hg3 row
    rw [hf] at hr
    linear_combination hr
